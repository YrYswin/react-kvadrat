...
const Modal: React.FC<Props> = ({ isOpen, setIsOpen }) => {
  ...
  const stopPropagation = (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    e.stopPropagation(); // Предотвращает закрытие при клике на содержимое модального окна
  };
  ...
  return (
    <div onClick={() => setIsOpen(false)} className="fixed inset-0 z-[99] h-full bg-black bg-opacity-50 mt-[65px]">
      <div
        onClick={stopPropagation}
        className="w-[219px] h-[380px] bg-red-700 text-md text-white absolute rounded-l-b-md rounded-bl-3xl right-0 py-[19px] z-[100]">
